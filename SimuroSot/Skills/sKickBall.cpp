...
namespace MyStrategy
{
	...
	void SkillSet::kickBall(const SParam &param)
  {
	...
	//  if (abs(state->ballPos.x)>HALF_FIELD_MAXX ||abs(state->ballPos.y)>HALF_FIELD_MAXY || abs(state->ballVel.x)>10000 || abs(state->ballVel.y)>10000) return;
	  float d=(HALF_FIELD_MAXX-GOAL_DEPTH)/10000;
     float m ;
		if(abs(state->ballVel.x )< 10)
			m = 0 ;
		else
			m = state->ballVel.y/state->ballVel.x;
		float xpos[101]={0},ypos[101]={0};
		float time_error[101]={0};
	  int minimum ,vbot=60;
	   Vector2D<int> dest;
	  ...
	  if(abs(state->ballVel.x)>30)
	  {
			  for(int i=1;i<100;i++)
			  {
				 ...
				 if(abs(state->ballVel.x)>10)
			      ...
			      tx1=0;
				 if(abs(state->homeVel[botID].x)>10)
				  tx2=abs((xpos[i]-state->homePos[botID].x)/(vbot*cos(atan(m))));
				 else
			      tx2=0;
				 if(abs(state->ballVel.y)>10)
			      ...
			      ty1=0;
				 if(abs(state->homeVel[botID].y)>10) 
				  ty2=abs((ypos[i]-state->homePos[botID].y)/(vbot*sin(atan(m))));
				 else
			      ty2=0;
